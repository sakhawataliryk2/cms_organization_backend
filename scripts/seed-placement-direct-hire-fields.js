/**
 * Seed up to 500 fields for Placements Direct Hire (field management).
 * Ensures total count is exactly 500: if 4 already exist, adds 496 (Field_1..Field_500, skipping existing names).
 * Field name and label: Field_1, Field_2, ... Field_500 (same as other modules). New ones are hidden.
 * Run from backend root: node scripts/seed-placement-direct-hire-fields.js
 */
require("dotenv").config({ path: require("path").resolve(__dirname, "../.env") });

const { getPool } = require("../config/getPool");

const ENTITY_TYPE = "placements-direct-hire";
